...
@objc(FIRStorage) open class Storage: NSObject {
   ...
   */
    //@objc(storageWithURL:) open class func storage(url: String) -> Storage? { return Storage.storage(app: FirebaseApp.app()!, url: url)
    ...
    open class func storage(url: String) -> Storage? {
        if let app = FirebaseApp.app() {
            return Storage.storage(app: app, url: url)
        } else {
            return nil
        }
    }
